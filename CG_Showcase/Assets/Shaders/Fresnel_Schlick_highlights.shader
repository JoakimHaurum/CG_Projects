Shader "Custom/Fresnel_Schlickk" {
		Properties{
			_Color("Color", Color) = (1,1,1,1)
			_SpecColor("Specular Color", Color) = (1,1,1,1)
			_Shininess("Specular Shininess", Float) = 10
			_Schlick("Fresnal higlight intensity", Float) = 5.0
			...
			float4 frag(vertOut i) : SV_TARGET{
				...
				if (dot(normalDir, lightDir) >= 0.0) {
					float halfWay = normalize(viewDir + lightDir);
					float schlick_approx = pow(1.0 - max(0.0, dot(halfWay, viewDir)), 5.0);
					float fresnal_factor = lerp(_SpecColor.rgb, float3(1.0, 1.0, 1.0), schlick_approx);
					specularReflection = attenuation * _LightColor0.rgb * fresnal_factor * pow(max(0.0, dot(reflect(-lightDir, normalDir), viewDir)), _Shininess);
				}
