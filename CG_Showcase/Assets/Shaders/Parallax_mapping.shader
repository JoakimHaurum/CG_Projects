Shader "Custom/Parallax_mapping" {
	Properties{
		_Color("Color", Color) = (1,1,1,1)
		_SpecColor("Specular material color", Color) = (1,1,1,1)
		_Shininess("Shininess of specular component", Float) = 1.0
		_MaxTexCoordOffset("Maximum tex coord offset", Float) = 0.01
		_Parallax("Max height", Float) = 0.01
		_MainTex("Main texture", 2D) = "white" {}
		_ParallaxMap("Parallax/Height map (alpha  comp)", 2D) = "black" {}
		...
		SubShader{
			Pass{
				...
				struct vertOut {
					...
					float3 viewWorld : TEXCOORD5;
					float3 viewScaledSurface : TEXCOORD6;
				};
				...
				vertOut vert(vertIn i) {
					...
					//Convert view direction from world space to local surface space / texture space
					float3 binormal = cross(i.normal, i.tangent.xyz)*i.tangent.w;
					float3 viewDirObj = mul(unity_WorldToObject, float4(_WorldSpaceCameraPos,1.0)).xyz - i.pos.xyz;
					float3x3 localSurf2ObjTrans = float3x3(i.tangent.xyz, binormal, i.normal);
					o.viewScaledSurface = mul(localSurf2ObjTrans, viewDirObj);

					o.worldPos = mul(unity_ObjectToWorld, i.pos);
					o.viewWorld = normalize(_WorldSpaceCameraPos - o.worldPos.xyz);
					o.tex = i.texcoord;
				...
				float4 frag(vertOut i) : SV_TARGET{
					//Parallax map part
					float height = _Parallax * (tex2D(_ParallaxMap, _ParallaxMap_ST.xy * i.tex.xy + _ParallaxMap_ST.zw).x - 0.5);
					float2 texCoordOffset = clamp(height*i.viewScaledSurface.xy/ i.viewScaledSurface.z, -_MaxTexCoordOffset, _MaxTexCoordOffset);
					...
					float4 texNormal = tex2D(_BumpMap, _BumpMap_ST.xy * (i.tex.xy + texCoordOffset) + _BumpMap_ST.zw); //Stored as 2-component encoded local surface coordiantes
